<div align="center">
  <h1>Carlos Enrique Gutiérrez Chimal</h1>
  <p><b>Full Stack Blockchain Developer & Solidity Engineer</b></p>
  <p>📍 Guadalajara, Mexico | 📱 +52 331 893 0280 | ✉️ cchimal.gutierrez@gmail.com</p>
  <p>
    <a href="https://www.linkedin.com/in/carlos-enrique-guti%C3%A9rrez-chimal">LinkedIn</a> • 
    <a href="https://github.com/k2gutierrez">GitHub</a> •
    <a href="URL_OF_YOUR_AMPLIFY_WEBSITE_HERE">Portfolio Website</a>
  </p>
</div>

---

### 🚀 Summary
Industrial Engineer and Full Stack Developer with a Master's in Data Science and AI, specializing in Web3. Expertise in engineering secure Solidity smart contracts and building scalable decentralized applications (dApps) using Next.js, Wagmi, and RainbowKit. Proven track record of leading smart contract integrations for Web3 organizations and driving enterprise-level technological solutions.

---

### 💻 Core Skills
| Category | Technologies |
| :--- | :--- |
| **Web3 & Blockchain** | Solidity, Foundry, Smart Contracts, Wagmi, RainbowKit, ZKPs |
| **Frontend & Backend** | Next.js, React, TypeScript, Supabase, Python |
| **Methodologies** | AI Integration (CrewAI), Smart Contract Security, Project Management |

---

### 💼 Experience

#### **Full Stack Blockchain Developer** | *MINGLES NFT*
*Jan 2022 – Present | Guadalajara, Mexico*
* Engineer and deploy secure smart contracts on the Ethereum blockchain using Solidity.
* Develop and maintain modern, responsive decentralized applications (dApps) using Next.js.

#### **Web3 & Smart Contract Developer** | *Comorph3*
*Jan 2022 – Feb 2025 | Guadalajara, Mexico*
* Co-founded a specialized development agency delivering end-to-end Web3 projects and smart contract solutions for external clients.

#### **Project Manager** | *CEDEM - Business Owner Center*
*May 2020 – Present | Guadalajara, Mexico*
* Lead internal IT operations, including the architecture of custom management systems and web infrastructure.
* Direct consulting project logistics and operational workflows.

#### **Production Manager** | *Escala Brewery*
*Jan 2017 – Jan 2020 | Guadalajara, Mexico*
* Served as production manager and quality engineer, overseeing brewing operations.

#### **Supply Chain Manager** | *ETT'O electronics*
*Jan 2016 – Jan 2017 | Guadalajara, Mexico*
* Managed supply chain operations including planning, purchasing, negotiating, and controlling all aspects of production in the SMT line.

#### **Founder** | *Brewhouse*
*Aug 2012 – Aug 2017 | Guadalajara, Mexico*
* Founded and operated a business venture featuring a commercial bar and craft beer production.

#### **Quality Engineer** | *LOMEDIC, S.A. DE C.V.*
*Jan 2011 – Jan 2012 | Zapopan, Mexico*
* Executed internal audits and enforced continuous improvement protocols for ISO9001:2010 compliance.

---

### 🎓 Education & Certifications

**Degrees:**
* **Master in Blockchain Development** - *Blockchain Accelerator*
* **B.S. Industrial Engineering** - *Universidad Tecmilenio*

**Certifications:**
* Foundry Fundamentals
* Fundamentals of Zero-Knowledge Proofs (ZKPs)
* Cybersecurity – *EUNCET, Barcelona*
* Full Stack Developer with React – *University of Hong Kong*
* EF SET English (C1 Advanced)